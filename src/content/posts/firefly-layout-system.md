---
title: HanYion 布局系统详解
published: 1970-01-03
description: 深入了解 HanYion 的布局系统，包括侧边栏布局（左侧/双侧）和文章列表布局（列表/网格），以及自适应网格列数。
image: ./images/firefly1.avif
tags: [HanYion, 布局, 博客, 使用指南]
category: 博客指南
draft: false
---
...
HanYion 提供了灵活的布局系统，允许您根据内容需求和个人喜好自定义博客的视觉呈现方式。布局系统主要包括**侧边栏布局**和**文章列表布局**两个维度，它们相互配合，共同决定了页面的整体结构。

本文将详细介绍 HanYion 的各种布局模式、它们的特点、使用场景，以及不同布局组合的效果。
...
侧边栏是博客页面的重要组成部分，用于展示导航、分类、标签、统计信息等辅助内容。HanYion 支持两种侧边栏布局模式。
...
文章列表是博客首页和归档页的核心内容，HanYion 提供两种展示方式，并支持多种网格配置。
...
HanYion 的网格模式内置了智能瀑布流布局支持，解决了网格布局中因图文混合文章导致的卡片高度不一致导致的空白问题。
...
HanYion 允许您自由组合侧边栏和文章列表布局。以下是各种组合的效果说明。
...
HanYion 的布局系统具有智能的响应式设计，会根据屏幕尺寸自动调整。
...
HanYion 的布局系统给予了您更大的自由度，您都可以通过简单的配置实现。
...
- 🏠 [HanYion 官方文档](https://docs-firefly.cuteleaf.cn/)
- ⭐ [HanYion GitHub](https://github.com/CuteLeaf/HanYion)
